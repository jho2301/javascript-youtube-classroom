...
const videoInfos = {
  ...
  async init() {
    ...
    const latestVideoInfos = oldVideoInfos.length ? await updateVideoInfos(oldVideoInfos) : [];
...
function savedVideoListTemplate(savedVideoInfos = []) {
  return [...savedVideoInfos].map(item => videoSnippetTemplate(item, controlButtonsTemplate(item.isWatched))).join('');
}
